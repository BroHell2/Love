...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  // Переменные для бегающей кнопки "НЕТ"
  // (Объявлены в начале, чтобы избежать Temporal Dead Zone)
  // ==========================================
  let dodgeStep = 0;
  ...
  // Синхронный мгновенный идентификатор (без задержек сети)
  let currentClientIp = localStorage.getItem('last_known_ip') || getFallbackClientId();

  function saveSession(ip, updates) {
    const activeIp = ip || currentClientIp;
    ...
    const existing = map[activeIp] || { ip: activeIp, history: [] };
    const merged = {
      ...existing,
      ...updates,
      device: deviceType,
  ...
  function displayFinalResult(answer, isLocked = false) {
    ...
    // Кнопка возврата доступна всегда, чтобы интерфейс не становился тупиковым
  ...
  // ==========================================
  // Мгновенная инициализация состояния UI
  // ==========================================
  ...
  // ==========================================
  // Асинхронное фоновое определение внешнего IP
  // (не блокирует показ страницы и не создает задержек)
  // ==========================================
  ...
  fetchIp().then((realIp) => {
    if (realIp && realIp !== currentClientIp) {
      const currentMap = getSessionsMap();
      // Если были сохранены данные под временным fallback ID — переносим
      if (currentMap[currentClientIp]) {
        currentMap[realIp] = { ...currentMap[currentClientIp], ip: realIp };
        delete currentMap[currentClientIp];
        localStorage.setItem(STORAGE_KEY, JSON.stringify(currentMap));
      }
      currentClientIp = realIp;
      localStorage.setItem('last_known_ip', realIp);

      // Если для реального IP уже было прохождение, обновляем состояние
      const realSession = currentMap[realIp];
      if (realSession && realSession.status === 'completed' && !slideResult.classList.contains('active')) {
        displayFinalResult(realSession.answer, true);
      }
  ...
  // ==========================================
  // Кнопка возврата: возвращает на Слайд 1 и сбрасывает статус
  // ==========================================
  btnReturn.addEventListener('click', () => {
    const map = getSessionsMap();
    if (map[currentClientIp]) {
      delete map[currentClientIp];
      localStorage.setItem(STORAGE_KEY, JSON.stringify(map));
    }
    showSlide(slide1);
  });
